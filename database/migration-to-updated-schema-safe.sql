-- Safe Migration Script: Update to Artist/Producer + Studio Schema
-- This version handles existing constraint violations properly

-- Step 1: First, let's see what data we have in invites table
-- Run this to check current invite codes:
-- SELECT DISTINCT code_type FROM public.invites;

-- Step 2: Add new columns to existing profiles table
...
-- Step 3: Update role values (migrate existing data) FIRST
-- Convert 'artist' and 'producer' to 'artist_producer'
UPDATE public.profiles 
SET role = 'artist_producer' 
WHERE role IN ('artist', 'producer');

-- Step 4: Update invites table BEFORE changing constraints
-- Handle all possible existing code_type values
UPDATE public.invites 
SET code_type = 'artist_producer' 
WHERE code_type IN ('artist', 'producer');

-- Handle any other unexpected code_type values - convert them to artist_producer as default
UPDATE public.invites 
SET code_type = 'artist_producer' 
...
-- Step 6: Create new tables for enhanced functionality
...
-- Session participants table (only create if bookings table exists)
DO $$ 
BEGIN
...
-- Create triggers for new tables (only if update_at function exists)
DO $$ 
BEGIN
...
-- Step 9: Update existing profiles with new structure
...
-- Add session_participants RLS only if the table was created
DO $$ 
BEGIN
...
DO $$ 
BEGIN 
    RAISE NOTICE 'Safe migration completed successfully! Your database has been updated to support the new artist/producer + studio structure.';
END $$; 